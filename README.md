...
`tools/list` at `https://gateway.pipeworx.io/arcgis-glynncounty/mcp` returns the tools in the table
...
```bash
curl -X POST https://gateway.pipeworx.io/v1/tools/arcgis_glynncounty_search_datasets \
  -H 'Content-Type: application/json' \
  -d '{"query":"parcels"}'
```

No account needed for the first calls. Inspect any tool: `GET https://gateway.pipeworx.io/v1/tools/arcgis_glynncounty_search_datasets`. Find one: `POST https://gateway.pipeworx.io/v1/tools/search_packs` with `{"query":"..."}`.
...
```json
{
  "mcpServers": {
    "arcgis-glynncounty": {
      "command": "npx",
      "args": ["-y", "@pipeworx/mcp-arcgis-glynncounty"]
    }
  }
}
```
...
```bash
npx -y @pipeworx/mcp-arcgis-glynncounty
```
